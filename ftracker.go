...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	rSpeed := meanSpeed(action, duration)
	return ((runningCaloriesMeanSpeedMultiplier * rSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	wSpeed := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + math.Pow(wSpeed, 2)/(height/100)*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	sSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (sSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
